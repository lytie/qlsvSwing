...
public class StudentView extends JFrame implements ActionListener, ListSelectionListener {
    ...
    private void initComponents() {
        ...
        dobMessLabel = new JLabel("(dd/MM/yyyy)");
        ...
        layout.putConstraint(SpringLayout.WEST, gmailLabel, 10, SpringLayout.WEST, panel);
        layout.putConstraint(SpringLayout.NORTH, gmailLabel, 200, SpringLayout.NORTH, panel);
        layout.putConstraint(SpringLayout.WEST, gpaLabel, 10, SpringLayout.WEST, panel);
        layout.putConstraint(SpringLayout.NORTH, gpaLabel, 230, SpringLayout.NORTH, panel);
        ...
        layout.putConstraint(SpringLayout.WEST, gmailField, 100, SpringLayout.WEST, panel);
        layout.putConstraint(SpringLayout.NORTH, gmailField, 200, SpringLayout.NORTH, panel);
        layout.putConstraint(SpringLayout.WEST, gpaField, 100, SpringLayout.WEST, panel);
        layout.putConstraint(SpringLayout.NORTH, gpaField, 230, SpringLayout.NORTH, panel);

        layout.putConstraint(SpringLayout.WEST, dobMessLabel, 170, SpringLayout.WEST, panel);
        layout.putConstraint(SpringLayout.NORTH, dobMessLabel, 70, SpringLayout.NORTH, panel);

        layout.putConstraint(SpringLayout.WEST, jScrollPaneStudentTable, 400, SpringLayout.WEST, panel);
    ...
    public void fillStudentFromSelectedRow() {
        ...
        if (row >= 0) {
            ...
            gmailField.setText(studentTable.getModel().getValueAt(row, 4).toString());
    ...
    public void showStudent(Student student) {
        ...
        ageField.setText("" + student.getDob());
        addressTA.setText(student.getAddress());
        gmailField.setText(student.getEmail());
    ...
    public Student getStudentInfo() {
        // validate student
        if (!validateName() || !validateAge() || !validateAddress() || !validateGPA() || validateGmail()) {
            return null;
        }
        try {
            ...
            Date dateDob = new SimpleDateFormat("dd/MM/yyyy").parse(ageField.getText().trim());
            student.setDob(dateDob);
    ...
    private boolean validateAge() {
        try {
            String dobText = ageField.getText();
            if (dobText == null || "".equals(dobText.trim())) {
                ageField.requestFocus();
                showMessage("D.o.B không được trống.");
            }

            Date dateDob = new SimpleDateFormat("dd/MM/yyyy").parse(dobText.trim());

        } catch (Exception e) {
            ageField.requestFocus();
            showMessage("D.o.B không hợp lệ!");
            return false;
        }
        return true;
    }
    ...
    private boolean validateGmail() {
        ...
        try {
            String gmailText = gmailField.getText();
            if (gmailText == null || "".equals(gmailText.trim())) {
                gmailField.requestFocus();
                showMessage("Email không được trống.");
            } else {
                matcher = pattern.matcher(gmailText.trim());
                if (!matcher.matches()) {
                    gmailField.requestFocus();
